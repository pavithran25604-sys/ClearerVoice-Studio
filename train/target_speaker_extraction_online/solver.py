...
class Solver(object):
    def __init__(self, args, model, optimizer, train_data, validation_data, test_data):
        ...
        self.stft_loss = loss_wrapper('stft')
    ...
    def _run_one_epoch(self, data_loader, state='train'):
        ...
        for i, (a_mix, a_tgt, ref_tgt) in enumerate(data_loader):
            ...
            if self.args.network_audio.tcn_attractor:
                a_tgt_est_tmp = self.model(a_mix.clone(), ref_tgt.clone())
                loss = self.loss(a_tgt, a_tgt_est_tmp) + 0.25*self.stft_loss(a_tgt, a_tgt_est_tmp)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.clip_grad_norm)
                self.optimizer.step()
                self.optimizer.zero_grad()

                a_reference = torch.zeros_like(a_tgt_est_tmp)
                a_reference[:, self.args.network_audio.window:] += a_tgt_est_tmp[:,:-self.args.network_audio.window].clone().detach()
                a_tgt_est = self.model(a_mix, ref_tgt, reference = a_reference)
                
                loss = self.loss(a_tgt, a_tgt_est) + 0.75*self.stft_loss(a_tgt, a_tgt_est)
