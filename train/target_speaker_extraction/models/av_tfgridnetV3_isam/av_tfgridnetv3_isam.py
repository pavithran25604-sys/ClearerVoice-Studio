...
class av_TFGridNetV3_isam(AbsSeparator):
    ...
    def forward(self, input, visual):
        input = torch.repeat_interleave(input.unsqueeze(1), repeats=self.args.speaker_no, dim=1)
        input = input.view(input.size(0)*self.args.speaker_no, input.size(2))
        ...
        batch = batch.view(batch.size(0)//self.args.speaker_no, self.args.speaker_no, batch.size(1))
        return batch
